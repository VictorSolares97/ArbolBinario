...
public class CD_Departamento
{
    ...
    public DataTable MtdConsultarDepartamentoPorNombre(string nombreDepartamento)
    {
        DataTable dt = new DataTable();
        ...
        {
            throw new Exception("Error al consultar departamento: " + ex.Message);
        }

        return dt;
    }

    public DataTable MtdConsultarDepartamentoPorMunicipio(string nombreMunicipio)
    {
        string Query = "SELECT d.Id, d.Nombre, d.DistanciaCapital, d.DepartamentoVecino " +
                       "FROM Departamento d " +
                       "INNER JOIN Municipio m ON d.Id = m.DepartamentoId " +
                       "WHERE m.Nombre = @NombreMunicipio";

        SqlDataAdapter adapter = new SqlDataAdapter(Query, conn.MtdAbrirConexion());
        adapter.SelectCommand.Parameters.AddWithValue("@NombreMunicipio", nombreMunicipio);
    ...
    public DataTable MtdObtenerDepartamentoPorMunicipio(string nombreMunicipio)
    {
        string Query = "SELECT d.Id AS DepartamentoId, d.Nombre, d.DistanciaCapital, d.DepartamentoVecino " +
                       "FROM Departamento d " +
                       "INNER JOIN Municipio m ON d.Id = m.DepartamentoId " +
                       "WHERE m.Nombre = @NombreMunicipio";
